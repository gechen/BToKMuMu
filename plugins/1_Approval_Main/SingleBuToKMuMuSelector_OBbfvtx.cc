...
//	if (datatype != "data") SaveGen();  ///// 13-08-2014 /////////////////////// for Signal MC ////////////////////////////////////////////////
	int index = SelectB(cut);
for (int i = 0; i< nb; i++) {
	int best_idx = -1;
	int tt = -1;
	double best_bvtxcl = 0.0;
	if (cut == "cut1_OBbfvtx") {  // MuID +	  Quality	+	Peak	+	OPT
	...
	if ( i != index && i != -1 && i != -2 && ( datatype == "data" || istruebu->at(i) )) {
		n_selected_ += 1;
		SaveEvent(i);
...
int SingleBuToKMuMuSelector::SelectB(string cut) {
	...
	}else if (cut == "cut1_OBbfvtx") {      // MuID +	Quality	+	Peak	+	OPT +	Single	B	+	Anti
		for (int i = 0; i< nb; i++) {
			if ( ! HasGoodMuons(i) ) continue;
			if ( ! QualitySelections(i) ) continue;
			if ( ! PeakCutSelections(i) ) continue;
	...
	}else if (cut == "cut1") {      // MuID +	Quality	+	Peak	+	OPT +	Single	B	+	Anti
		for (int i = 0; i< nb; i++) {
			if ( ! HasGoodMuons(i) ) continue;
			if ( ! QualitySelections(i) ) continue;
			if ( ! PeakCutSelections(i) ) continue;
	...
	}else if (cut == "cut1_test") {  // MuID +	Single	B +	Quality	+	Peak	+	OPT +	Anti
		for (int i = 0; i< nb; i++) {
			if ( ! HasGoodMuons(i) ) continue;
			if (bvtxcl->at(i) > best_bvtxcl) {
				best_bvtxcl = bvtxcl->at(i);
