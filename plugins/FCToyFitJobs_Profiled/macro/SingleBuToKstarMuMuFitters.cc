...
void angular3D_bin(int iBin, const char outfile[] = "angular3D", double dataScaleFactor = 1, double fixFl = -1, double fixAfb = -1)
{//{{{
...
void harvestFCFitResult(int iBin, int nToy=500, double newAfb=-1, double newFl=-1)
{//{{{
    // Buffers for checking directory/FILE
    struct stat fiBuff;

    // Setting
    TString otoyspath = TString::Format("./limit/validation/bin%d",iBin);
    const int nHistBins   = 2000;
    ...
    // harvest
    if (newAfb != -1) thisAfb=newAfb;
    if (newFl  != -1) thisFl =newFl;

    if (stat(TString::Format("%s/afb%+04.0f_fl%+04.0f",otoyspath.Data(),thisAfb*1000,thisFl*1000).Data(),&fiBuff) != 0) return;
    owspacepath=TString::Format("%s/afb%+04.0f_fl%+04.0f",otoyspath.Data(),thisAfb*1000,thisFl*1000);
    ...
    if (stat(TString::Format("%s/setSummary.root",owspacepath.Data()).Data(),&fiBuff) == 0){
        printf("INFO\t\t: \"setSummary.root\" exists, replace fit result!\n");
        fout = new TFile(TString::Format("%s/setSummary.root",owspacepath.Data()),"UPDATE");
        h_setSummaryAfb = (TH1F*)fout->Get("h_setSummaryAfb");
        h_setSummaryFl  = (TH1F*)fout->Get("h_setSummaryFl" );
    }else{
        fout = new TFile(TString::Format("%s/setSummary.root",owspacepath.Data()),"RECREATE");
        h_setSummaryAfb = new TH1F("h_setSummaryAfb", TString::Format("h_afb%+04.0f",thisAfb*1000).Data(), nHistBins, -1.,1.);
        h_setSummaryFl  = new TH1F("h_setSummaryFl", TString::Format("h_fl%+04.0f",thisFl*1000).Data(), nHistBins, 0.,1.);
    ...
    TF1 *afbPDFs = new TF1(TString::Format("afbPdf_%+04.0f",thisAfb*1000).Data(),"gaus(0)+[3]*exp(-0.5*(x/[4])**2)", -0.70,0.70);
    ...
    TF1 *flPDFs = new TF1(TString::Format("flPdf_%+04.0f",thisFl*1000).Data(),"gaus(0)+[3]*exp(-0.5*(x/[4])**2)+[5]*exp(-0.5*((x-0.995)/[6])**2)", 0,1);
    ...
    h_setSummaryFl->Draw();
    h_setSummaryFl->SaveAs(TString::Format("%s/h_setSummaryFl.cc",owspacepath.Data()));
    fout->Write(TString::Format("%s/setSummary.root",owspacepath.Data()).Data(),TObject::kOverwrite);
    fout->Close();
}//}}}
...
{//{{{
    ...
    for(int iTrueAfb = 0; iTrueAfb*stepSizeAfb < 2.; iTrueAfb++){
        ...
        finAfb[iTrueAfb] = new TFile(TString::Format("%s/afb%+04.0f_fl%+04.0f/setSummary.root",otoyspath.Data(),thisAfb*1000,thisFl*1000).Data(),"READ");
        h_setSummaryAfb[iTrueAfb] = (TH1F*)finAfb[iTrueAfb]->Get("h_setSummaryAfb");
        h_setSummaryAfb[iTrueAfb]->SetName(TString::Format("h_setSummaryAfb%+04.0f_fl%+04.0f",thisAfb*1000,thisFl*1000).Data());
    ...
    for(int iTrueFl = 0; iTrueFl*stepSizeFl < 1.; iTrueFl++){
        ...
        finFl[iTrueFl] = new TFile(TString::Format("%s/afb%+04.0f_fl%+04.0f/setSummary.root",otoyspath.Data(),thisAfb*1000,thisFl*1000).Data(),"READ");
        h_setSummaryFl[iTrueFl] = (TH1F*)finFl[iTrueFl]->Get("h_setSummaryFl");
        h_setSummaryFl[iTrueFl]->SetName(TString::Format("h_setSummaryFl%+04.0f_afb%+04.0f",thisFl*1000,thisAfb*1000).Data());
...
//printf("DEBUG\t\t: h_LRFl(%d) = %f\n",iFl,h_LRFl[iTrueFl]->GetBinContent(iFl));
    ...
    g_flIntervalHi->SetTitle("");
    g_flIntervalHi->GetXaxis()->SetTitle("Measured F_{L}");
    g_flIntervalHi->GetXaxis()->SetLimits(0.,1.1*max(fl,flTrueValues[nFlPoints-1]));
    g_flIntervalHi->GetYaxis()->SetTitle("True F_{L}");
    ...
    g_afbIntervalHi->SetMarkerSize(2);
    g_afbIntervalLo->SetMarkerSize(2);
    g_afbIntervalHi->Draw("AP");
    g_afbIntervalLo->Draw("P SAME");
    line->SetLineStyle(2);
    line->SetLineColor(2);
...
void createFCToys2(int iBin, int nToy=500)// Create toys with profile likelihood considered
{//{{{
    // Buffers for checking directory/FILE
    struct stat fiBuff;
    FILE *fi = 0;
    ...
    for(int iAfb = 0; iAfb*stepSizeAfb < 2.; iAfb++){
        ...
        nsigInToys          = 0;
        nbkgCombInToys      = 0;
        nbkgjpsiPeakInToys  = 0;
        nbkgpsi2sPeakInToys = 0;
        nbkgjpsiPeakDataset = 0;
        nbkgpsi2sPeakDataset= 0;
        gaus_nbkgjpsiPeak   = 0;
        gaus_nbkgpsi2sPeak  = 0;
        ...
        double  fl_ubd  = readParam(iBin,"fl_migrad",0); 
        thisFl          = toBoundedFl(fl_ubd);
        thisFs          = readParam(iBin,"fs_migrad",0);
        thisAs          = toOriginAs(readParam(iBin,"as_migrad",0),thisFs,fl_ubd);
        double  nsig    = wspace->var("nsig")->getVal();
        double  nbkgComb= wspace->var("nbkgComb")->getVal();
        ...
        // Copy needed wspace, see http://stackoverflow.com/questions/3680730/c-fileio-copy-vs-systemcp-file1-x-file2-x

        // Generating
        // Remark: As generating combBkg toys, take nuisance parameter from "profiled" fit result.
        genToySignal(iBin, nsigInToys,thisAfb,thisFl,thisFs,thisAs);
        splitToySamples(TString::Format("%s/signalToy_Bin%d.root",owspacepath.Data(),iBin).Data(),nToy,nsigInToy,TString::Format("signalToy_Bin%d",iBin).Data());
        iwspacepath=owspacepath;
        genToyCombBkg(iBin,nbkgCombInToys);
        splitToySamples(TString::Format("%s/combBkgToy_Bin%d.root",owspacepath.Data(),iBin).Data(),nToy,nbkgCombInToy,TString::Format("combBkgToy_Bin%d",iBin).Data());
...
void createFCToy(int iBin, int nToy=500, double newAfb=-1, double newFl=-1)
{//{{{
    // Creates toys for a single point with large statistics to validate the distribution of fit results matches Gaussian
    ...
    printf("thisAfb=%+04f\n",thisAfb);
    if (!scanAfbFlPositivePdf(thisAfb,thisFl,true)){
        printf("Input afb%+04.0f_fl%+04.0f is not positive\n",thisAfb*1000,thisFl*1000);
        return;
    }
...
void scanProfile(int iBin, const char outfile[]="scanProfiled")
{//{{{
    ...
    TCanvas *canvas = new TCanvas("canvas");
    TGraph *gr_profiledFl  = new TGraph();
    TGraph *gr_profiledAfb = new TGraph();
    int nProfiledFl = 0;
    int nProfiledAfb= 0;

    double thisAfb, thisFl;
    struct stat fiBuff;
    for(int iAfb = 0; iAfb*stepSizeAfb < 2.; iAfb++){
        thisAfb = iAfb*stepSizeAfb-1.+stepSizeAfb/2;
        ...
        thisFl     = toBoundedFl(readParam(iBin,"fl_migrad",0));
        //thisFl     = toBoundedFl(readParam(iBin,"fl",0));
        gr_profiledFl->SetPoint(nProfiledFl,thisAfb,thisFl);
        
    }

    gr_profiledFl->GetXaxis()->SetTitle("True A_{FB}");
    gr_profiledFl->GetYaxis()->SetTitle("Profiled F_{L}");
    gr_profiledFl->Draw("AP");
    canvas->Print(TString::Format("./plots//%sFl_bin%d.pdf",outfile,iBin).Data());
...
int main(int argc, char** argv) {
    ...
    if (func == "bmass") {
        ...
        //createFCToys(whichBin,nToy);// fix fl/afb and perform simple 1D generation.
        ...
        //getFCInterval(whichBin,nToy);// minimal interval
        getFCInterval2(whichBin,nToy);//likelihood ratio ordering
    }else if (func == "createToys"){
        scaleFactor = 100;
        
        double expNCombBkg[nQ2Ranges] = { 0 , 0 , 0 , 0 , 61.3 , 0 , 0 , 0 , 0 , 52.3 , 133.5 , 0 , 0};
        int    expNSig[nQ2Ranges]     = { 0 , 0 , 0 , 0 , 28   , 0 , 0 , 0 , 0 , 53   , 25    , 0 , 0};
        ...
        for (int iBin = 0; iBin < nSummaryBins; iBin++) {
            ...
            //angular3D_bin(summaryBin[iBin],"angular3D",scaleFactor);
        }
        accXrecoEff2(summaryBin[1],gKeepParam);
        angular2D_bin(summaryBin[1]);
        //angular3D_prior(12,"angular3D_prior",gKeepParam);
        //angular3D_bin(12,"angular3D",scaleFactor);
    }else if (func == "test"){
        ch->Add(infile.Data());
        ...
        //createFCToy(9,5000,-0.395, 0.444);
        //harvestFCFitResult(9,5000,-0.395,0.444);
        scanProfile(4);
        scanProfile(9);
        scanProfile(10);
    }else{ 
