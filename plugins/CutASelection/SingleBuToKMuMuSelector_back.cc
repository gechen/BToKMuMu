...
bool SingleBuToKMuMuSelector::PeakCutSelections(int i) {
	if ( // rejection of Jpsi & Psip peak  cut
		...
		( mumumass->at(i) < 3.096916-5. *mumumasserr->at(i) ) || ( ( mumumass->at(i) > 3.096916+5.0*mumumasserr->at(i) ) && ( mumumass->at(i) < 3.686109-5.*mumumasserr->at(i) ) )  || (mumumass->at(i) > 3.686109+5.0*mumumasserr->at(i)) // J/PSi && PSi(2S) cut
		) return true;
	return false;
}
...
bool SingleBuToKMuMuSelector::AntiPsipSelections(int i) {
	...
	//	(fabs( bmass->at(i) - mumumass->at(i) - 1.593 ) < 0.055)  //0.08 // Only PSi(2S)
		) return true;
	return false;
}
...
/*		   (trkpt->at(i) > 0.8)  
		&& (bvtxcl->at(i) > 0.01)  
		&& (( blsbs->at(i) / blsbserr->at(i) ) > 3.7)
		&& (bcosalphabs2D->at(i) > 0.9983)
		&& (fabs( (trkdcabs->at(i))/(trkdcabserr->at(i)) )> 0.8)
*/	/*	   (trkpt->at(i) > 1.3)  
		&& (bvtxcl->at(i) > 0.08)  
