...
bool SingleBuToKMuMuSelector::AntiJpsiSelections(int i) {
	if ( // Anti Jpsi  cut
		(fabs( bmass->at(i) - mumumass->at(i) - 2.182 ) < 0.065)  //0.12 // Only JPSi
		) return true;
	return false;
}
bool SingleBuToKMuMuSelector::AntiPsipSelections(int i) {
	if ( // Anti Psip  cut
		(fabs( bmass->at(i) - mumumass->at(i) - 1.593 ) < 0.05)  //0.08 // Only PSi(2S)
		) return true;
	return false;
}
